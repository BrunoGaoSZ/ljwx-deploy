...
- `pgvector/pgvector:pg16`（内置 `vector` 扩展）
- 原因：`ljwx-chat` 等业务依赖 `CREATE EXTENSION vector`
...
# 测试网络
kubectl run -it --rm debug --image=pgvector/pgvector:pg16 --restart=Never -- \
...
```bash
# 更新镜像
kubectl -n infra set image statefulset/postgres postgres=pgvector/pgvector:pg16
